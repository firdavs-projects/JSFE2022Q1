...
import {CROSS_CHECK_PR} from './utils/constants';
...
interface IApp {
    ...
    initialProducts: Smartphone[];
    initialCart: Smartphone[];
    maxCartSize: number;
    onChangeInitialData?: (filters: FilterType, products: Smartphone[], cart: Smartphone[]) => void;
}
...
const App: FC<IApp> =
    ({
        ...
        const [cart, setCart] = useState<Smartphone[]>(initialCart);
        const [show, setShow] = useState<boolean>(false);
        const {filtered, filters, setFilters} = useFilters(products, initialFilters);
        ...
        useEffect((): void => {
            onChangeInitialData && onChangeInitialData(filters, filtered, cart);
        }, [filters, filtered, cart]);
        ...
        const handleAddToCart = useCallback((product: Smartphone): void => {
            if (cart.length === maxCartSize && !cart.find(p => p.id === product.id)) {
                ...
                {!isLoading && <FiltersLayout>
                    <FilterContainer lg={12}>
                        ...
                        <div className="d-flex align-items-center justify-content-between mt-3">
                            ...
                            <Brandbox
                                brands={removeDuplicates<Manufacturers>(initialProducts.map(s => s.manufacturer))}
                        ...
                        <div className="d-flex align-items-center justify-content-between mt-3">
                            ...
                            <Countbox
                                counts={removeDuplicates<number>(initialProducts.map(s => s.camCount)).sort()}
                    ...
                    <FilterContainer>
                        ...
                        <div className="d-flex">
                            <Button
                                ...
                                onClick={() => setFilters({...initialFilters, sort: filters.sort})}
                            >
                                Сбросить фильтры
                            </Button>
                            <Button
